  ...
  var body: some View {
    VStack(spacing: 0) {
      GeometryReader { geo in
        ZStack(alignment: .center) {
          VStack(alignment: .center, spacing: 0) {
            ...
            Text("After the faceting process is completed, you need to color the vectorized parts that have been created according to the color spectrum of the original photo.")
      ...
      if #available(iOS 16.0, *) {
        NavigationLink(
          destination: PalleteScene(path: $path),
          isActive: $nextPage) {
